...
use log::{info, debug};
use crate::types::{AppError, SteamAppDetails, SteamSearchResult, SteamOwnedGame};
...
impl SteamApiClient {
    ...
    pub fn get_cdn_logo_url(app_id: &str) -> String {
        format!("https://shared.fastly.steamstatic.com/store_item_assets/steam/apps/{app_id}/logo.png")
    }
    ...
    pub fn fetch_app_details(&self, app_id: &str) -> Result<SteamAppDetails, AppError> {
        ...
        let resp = self.client.get(&url).send()
            .map_err(|e| AppError::Other(format!("Failed to connect to Steam Storefront: {e}")))?;

        if resp.status() == reqwest::StatusCode::TOO_MANY_REQUESTS {
            return Err(AppError::Other("Steam Storefront rate limit (429) hit. Please wait a moment.".into()));
        }

        if !resp.status().is_success() {
            return Err(AppError::Other(format!("Steam Storefront returned status: {}", resp.status())));
        }

        let root: Value = resp.json()
            .map_err(|e| AppError::Other(format!("Failed to parse Steam response: {e}")))?;

        let app_obj = root.get(app_id)
            .ok_or_else(|| AppError::Other(format!("App ID {app_id} not found in response")))?;

        let success = app_obj.get("success").and_then(|v| v.as_bool()).unwrap_or(false);
        if !success {
            return Err(AppError::Other(format!("Steam returned unsuccessful for App ID {app_id}")));
        }

        let data = app_obj.get("data")
            .ok_or_else(|| AppError::Other("Missing 'data' field in Steam response".into()))?;

        let name = data.get("name").and_then(|v| v.as_str()).unwrap_or("Unknown Game").to_string();
        ...
        let header_image = data.get("header_image")
            ...
            .unwrap_or("")
            .to_string();
        let capsule_image = data.get("capsule_image")
            .and_then(|v| v.as_str())
            .unwrap_or("")
            .to_string();

        let genres = data.get("genres")
        ...
        let publishers = data.get("publishers")
            .and_then(|v| v.as_array())
    ...
    pub fn search_store(&self, query: &str) -> Result<Vec<SteamSearchResult>, AppError> {
        let encoded_term = urlencoding(query);
        let url = format!("https://store.steampowered.com/api/storesearch/?term={encoded_term}&l=english&cc=US");
        ...
        if !resp.status().is_success() {
            return Err(AppError::Other(format!("Steam search returned status: {}", resp.status())));
        }
        ...
        let search_data: SearchResponse = resp.json()
            .map_err(|e| AppError::Other(format!("Failed to parse search response: {e}")))?;

        let results = search_data.items
            .into_iter()
            .map(|item| {
    ...
    pub fn detect_local_steam_id() -> Option<String> {
        ...
        let candidates: Vec<PathBuf> = steam_roots.iter().map(|p| p.join("config").join("loginusers.vdf")).collect();
    ...
    pub fn fetch_owned_games(&self, api_key: &str, steam_id: &str) -> Result<Vec<SteamOwnedGame>, AppError> {
        let url = format!(
        ...
        if !resp.status().is_success() {
            return Err(AppError::Other(format!("Steam Web API error status: {}", resp.status())));
        }
        ...
        let body: WebApiResponse = resp.json()
            .map_err(|e| AppError::Other(format!("Failed to parse GetOwnedGames: {e}")))?;

        let games = body.response.games.into_iter().map(|g| {
            SteamOwnedGame {
                appid: g.appid,
                name: g.name,
